...
```
This function fits a logistic regression model using maximum likelihood estimation. It is designed to handle both numerical and categorical variables by internally converting categorical variables into dummy variables if necessary. The function offers an optional bagging mechanism to enhance the robustness of the model fit, particularly useful in datasets susceptible to overfitting or with high variance.
...
Details:
This function uses the glm function from the base R stats package with a binomial family and a logit link function to fit the logistic regression model. It calculates the model's coefficients, their standard errors, z-values, p-values, and the AIC.

Value:
Returns a detailed list similar to the logistic function, containing model coefficients, standard errors, z-values, p-values, AIC value, and a summary table of the model.
